...
# build the napari viewer and set up the click-to-predict behaviour.
def run_gui(query_stack, target_stack, model, device,
            embeddings_tensor, candidate_points_target):
    viewer = napari.Viewer()

    query_layer = viewer.add_image(query_stack, name=f"{STACK_NAME} frame {QUERY_FRAME} - Query")
    viewer.add_image(target_stack, name=f"{STACK_NAME} frame {TARGET_FRAME} - Target")
    ...
    def on_click_query_stack(layer, event):
        ...
        # green dot: the model's prediction. Both frames are drawn on top of each other, so no position shift is needed.
    ...
    # attach the click handler to the query image.
    query_layer.mouse_drag_callbacks.append(on_click_query_stack)

    # start the viewer window and wait for clicks.
    napari.run()
